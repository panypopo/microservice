...
session1执行查询,两次查询得到的结果一样.它看不到session4对数据库的修改,虽然session4的事务已经提交.这是因为session4的事务是在session1的事务之后才开始.从这里也可以看出,repeatable read实现了可重复读。
...
同样,这个会话查询到的结果与会话1和会话2的结果也不一样.而且会话3看到了会话4对数据库的修改.
